...
public:
    ListNode *detectCycle(ListNode *head) {
        ...
        while(curr!=nullptr)
        {
            if(find(NodeArray.begin(),NodeArray.end(),curr)!=NodeArray.end())
                return curr;
...
自己的幽默解法第二弹：虽然比起解法一快了80%,但由于解法一实在太慢了所以解法二依旧很慢，以下是代码：33ms
...
public:
    ListNode *detectCycle(ListNode *head) {
        ...
        ListNode *curr = head;
        int circle = 0;
        while(fast!=nullptr && fast->next!=nullptr)//判断有没有环
        {
            fast = fast->next->next;
            slow = slow->next;
            if(fast == slow) break;
